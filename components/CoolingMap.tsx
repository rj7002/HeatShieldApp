...
const TYPE_EMOJI: Record<string, string> = {
  Library: "📚", "Community Center": "🏢", Pool: "🏊",
  "Park / Shade": "🌳", "Social Services": "🤝",
  Cinema: "🎬", "Shopping Mall": "🏬", "Recreation Center": "🏋️",
  "House of Worship": "⛪",
};

const TYPE_COLOR: Record<string, string> = {
  Library: "#3b82f6", "Community Center": "#8b5cf6", Pool: "#06b6d4",
  "Park / Shade": "#10b981", "Social Services": "#f59e0b",
  Cinema: "#ec4899", "Shopping Mall": "#6366f1", "Recreation Center": "#14b8a6",
...
export default function CoolingMap({ lat, lng, city, centers, uhiDelta = 3, tempF }: Props) {
  ...
  useEffect(() => {
    ...
    import("leaflet").then((L) => {
      ...
      const tempBadge = tempF
        ? `<div style="background:rgba(239,68,68,0.92);color:white;padding:2px 6px;border-radius:6px;font-size:11px;font-weight:900;margin-top:5px;text-align:center;white-space:nowrap;border:1px solid rgba(255,255,255,0.25);box-shadow:0 2px 8px rgba(0,0,0,0.5)">${tempF}°F</div>`
        : "";
      ...
      const userIcon = L.divIcon({
        ...
        iconSize: [18, tempF ? 46 : 18],
        iconAnchor: [9, tempF ? 46 : 9],
        className: "",
      });
      ...
      L.marker([lat, lng], { icon: userIcon })
        .addTo(map)
        .bindPopup(
          `<b style="color:#f97316;font-size:13px">${city}</b>${
            tempF ? `<br/><span style="font-size:12px"><b style="color:#fca5a5">${tempF}°F</b> · UHI +${uhiDelta}°F above rural</span>` : ""
          }`,
      ...
      centers.forEach((c) => {
        ...
        L.marker([c.lat, c.lng], { icon })
          .addTo(map)
          .bindPopup(
            `<b style="color:white;font-size:13px">${c.name}</b>
            ...
            { className: "hs-leaflet-popup", maxWidth: 240 }
          );
      });
      ...
      if (centers.length > 0) {
        const allPoints: [number, number][] = [
          [lat, lng],
          ...centers.map((c): [number, number] => [c.lat, c.lng]),
        ];
  ...
  }, [lat, lng, city, centers, uhiDelta, tempF]);
